// Structure principale du bot Discord avec les commandes : /casier, /rapport_inter, /renforts, /setup_heures
...
const { 
  ...
  TextInputStyle, Events, EmbedBuilder, ChannelType,
  PermissionsBitField, SelectMenuBuilder
} = require('discord.js');
...
client.on(Events.InteractionCreate, async interaction => {
  if (interaction.isChatInputCommand()) {
    ...
    if (commandName === 'casier') {
      ...
      collector.on('collect', async msg => {
        ...
        const btn = await interaction.channel.awaitMessageComponent({ filter: i => i.user.id === interaction.user.id && i.customId === 'continuer_officier', time: 60000 });
  ...
  if (interaction.isModalSubmit()) {
    ...
    if (id === 'modal_officier') {
      ...
      const btn = await interaction.channel.awaitMessageComponent({ filter: i => i.user.id === interaction.user.id && i.customId === 'continuer_suspect', time: 60000 });
...
client.on(Events.InteractionCreate, async interaction => {
  ...
  if (interaction.commandName === 'rapport_inter') {
  ...
  if (interaction.commandName === 'setup_heures') {
...
client.on(Events.InteractionCreate, async interaction => {
  ...
  if (interaction.customId === 'debut_service') {
